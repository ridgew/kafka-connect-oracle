...
public class OracleSourceTask extends SourceTask {
  ...
  public void start(Map<String, String> map) {
    ...
    // a connection to a database, etc.
    ...
    try {
      ...
      if (offset != null) {
        ...
        streamOffsetCommitScn = (commitScnPositionObject != null)
            ? Long.parseLong(String.valueOf(commitScnPositionObject))
            : 0L;
        streamOffsetRowId = (rowIdPositionObject != null) ? (String) offset.get(ROWID_POSITION_FIELD) : "";
        if (oraDeSupportCM)
          streamOffsetScn = streamOffsetCommitScn;
        log.info("Offset values , scn:{},commitscn:{},rowid:{}", streamOffsetScn, streamOffsetCommitScn,
            streamOffsetRowId);
      }
      ...
      log.info(String.format("Log Miner will start at new position SCN : %s with fetch size : %s", streamOffsetScn,
          config.getDbFetchSize()));
      if (!oraDeSupportCM) {
        ...
        // tLogMiner = new Thread(new
        // LogMinerThread(sourceRecordMq,dbConn,streamOffsetScn,
        // logMinerStartStmt,logMinerSelectSql,config.getDbFetchSize(),topic,dbName,utils));
        tLogMiner = new LogMinerThread(sourceRecordMq, dbConn, streamOffsetScn, logMinerStartStmt, logMinerSelectSql,
            config.getDbFetchSize(), topic, dbName, utils);
        // tLogMiner.start();
    ...
    try {
      ...
      if (!oraDeSupportCM) {
        while (!this.closed && logMinerData.next()) {
          ...
          if (skipRecord) {
            if ((scn.equals(streamOffsetCtrl)) && (commitScn.equals(streamOffsetCommitScn))
                && (rowId.equals(streamOffsetRowId)) && (!contSF)) {
              skipRecord = false;
            }
          ...
          topic = config.getTopic().equals("")
              ? (config.getDbNameAlias() + DOT + row.getSegOwner() + DOT
                  + (operation.equals(OPERATION_DDL) ? DDL_TOPIC_POSTFIX : segName)).toUpperCase()
              : topic;
          // log.info(String.format("Fetched %s rows from database %s
          // ",ix,config.getDbNameAlias())+" "+row.getTimeStamp()+" "+row.getSegName()+"
          // "+row.getScn()+" "+commitScn);
          if (ix % 100 == 0)
            log.info(String.format("Fetched %s rows from database %s ", ix, config.getDbNameAlias()) + " "
                + row.getTimeStamp());
          dataSchemaStruct = utils.createDataSchema(segOwner, segName, sqlRedo, operation);
          if (operation.equals(OPERATION_DDL))
            row.setSegName(DDL_TOPIC_POSTFIX);
          /**
           * Issue 68
           * 
           * Addition of DML types to target to allow only replication of certain DML
           * operations.
           */
          if (config.getDMLTypes() == null || config.getDMLTypes().equals("")
              || Arrays.asList(config.getDMLTypes().toUpperCase().split(",")).contains(operation)) {
            records.add(new SourceRecord(sourcePartition(), sourceOffset(scn, commitScn, rowId), topic,
                dataSchemaStruct.getDmlRowSchema(), setValueV2(row, dataSchemaStruct)));
            streamOffsetScn = scn;
          }
  ...
  private Struct setValueV2(Data row, DataSchemaStruct dataSchemaStruct) {
    Struct valueStruct = new Struct(dataSchemaStruct.getDmlRowSchema()).put(SCN_FIELD, row.getScn())
        .put(SEG_OWNER_FIELD, row.getSegOwner()).put(TABLE_NAME_FIELD, row.getSegName())
        .put(TIMESTAMP_FIELD, row.getTimeStamp()).put(SQL_REDO_FIELD, row.getSqlRedo())
        .put(OPERATION_FIELD, row.getOperation()).put(DATA_ROW_FIELD, dataSchemaStruct.getDataStruct())
