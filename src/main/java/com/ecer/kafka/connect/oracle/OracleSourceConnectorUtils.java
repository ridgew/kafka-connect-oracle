...
public class OracleSourceConnectorUtils {
  ...
  protected void parseTableWhiteList() {
    ...
    for (String tables : tabWithSchemas) {
      ...
      logMinerSelectWhereStmt += "(" + SEG_OWNER_FIELD + "='" + tabs.get(0) + "'"
          + (tabs.get(1).equals("*") ? "" : " and " + TABLE_NAME_FIELD + "='" + tabs.get(1) + "'") + ") or ";
    }
  ...
  protected void loadTable(String owner, String tableName, String operation) throws SQLException {
    ...
    // SchemaBuilder dataSchemaBuiler =
    // SchemaBuilder.struct().name((config.getDbNameAlias()+DOT+owner+DOT+tableName+DOT+"Value").toLowerCase());
    ...
    /*
     * if (config.getMultitenant()) {
     * mineTableCols=dbConn.prepareCall(sql.getContainerDictionarySQL()); } else {
    ...
    if (!mineTableColsResultSet.isBeforeFirst()) {
      // TODO: consider throwing up here, or an NPE will be thrown in
      // OracleSourceTask.poll()
    ...
    while (mineTableColsResultSet.next()) {
      ...
      if (dataType.contains(TIMESTAMP_TYPE))
      ...
      com.ecer.kafka.connect.oracle.models.Column column = new com.ecer.kafka.connect.oracle.models.Column(owner,
          tableName, columnName, nullable, dataType, dataLength, dataScale, pkColumn, uqColumn, columnSchema);
  ...
  protected Map<String, LinkedHashMap<String, String>> parseSql(String owner, String tableName, String sqlRedo)
     ...
     * the mapping here to return text already escaped. If a consumer that is
     * intending on replicating the data from Oracle source to Oracle destination
     * attempts to use or inspect data from the mapped values it will double escape
     * single quotes causing data to be different from source and potentially
     * over-running the size of the column causing errors.
     */
  ...
  protected DataSchemaStruct createDataSchema(String owner, String tableName, String sqlRedo, String operation)
      throws Exception {
    ...
    if (config.getParseDmlData() && (!operation.equals(OPERATION_DDL))) {
      if (!tableSchema.containsKey(tableKey)) {
        if (!tableName.matches("^[\\w.-]+$")) {
          throw new ConnectException("Invalid table name " + tableName
              + " for kafka topic.Check table name which must consist only a-z, A-Z, '0-9', ., - and _");
        }
      ...
      for (String col : beforeDataMap.keySet()) {
        ...
        beforeDataStruct.put(col,
            value.equals(NULL_FIELD) ? null : reSetValue(value, tabColsMap.get(keyTabCol).getColumnSchema()));
    ...
    Schema newSchema = SchemaBuilder.struct().name(preSchemaName).field(SCN_FIELD, Schema.INT64_SCHEMA)
        .field(SEG_OWNER_FIELD, Schema.STRING_SCHEMA).field(TABLE_NAME_FIELD, Schema.STRING_SCHEMA)
        .field(TIMESTAMP_FIELD, org.apache.kafka.connect.data.Timestamp.SCHEMA)
        .field(SQL_REDO_FIELD, Schema.STRING_SCHEMA).field(OPERATION_FIELD, Schema.STRING_SCHEMA)
        .field(DATA_ROW_FIELD, dataSchema).field(BEFORE_DATA_ROW_FIELD, dataSchema).build();
